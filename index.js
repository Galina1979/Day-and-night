...
//   день ночь
let changeThemeButtons = document.querySelectorAll('.Theme_svg');   //помещаем папки смены темы в переменную
changeThemeButtons.forEach(button => {
    button.addEventListener('click', function(){  //  к каждой добавляем обработчик событий на клик
        let theme = this.dataset.theme;  // помещаяем в переменную название темы из атрибута data-theme
       applyTheme(theme);   // вызываем функцию, которая меняет тему и передаём в неё её название
    });
});
function applyTheme(themeName){
   ...
   document.querySelector(`[data-theme="${themeName}"]`).style.display = 'none'; //  скрывает кнопку для активной темы
   localStorage.setItem('theme', themeName); // при обновлении страницы фон не сменится
}
